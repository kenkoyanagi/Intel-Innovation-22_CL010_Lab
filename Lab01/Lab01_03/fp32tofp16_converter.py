...
from onnxmltools.convert.common._container import ModelComponentContainer
import onnx
...
def get_block_initializer(graph, block_op_initializer, block_initializer):
    '''
    Get the initializers that we want to block according to the block_op
    '''
    nodes  = graph.node
    potential_inputs = []
    # get all inputs from the block _op
    for n in nodes:
        if n.op_type in block_op_initializer :
            for i in n.input:
                potential_inputs.append(i)
    # go through all initializers and find those that are inputs of the block_op
    for i in graph.initializer:
        if i.name in potential_inputs:
            block_initializer.append(i.name)

def convert_float_to_float16(model, block_op_initializer = [], block_initializer = [] ):
    '''
    ...
    # get block initializers  from block_op_intializer
    get_block_initializer(model.graph, block_op_initializer, block_initializer)
        ...
        for q in queue:
            ...
            if isinstance(q, onnx_proto.GraphProto):
                for n in q.node:
                    ...
                    if n.op_type in op_black_list:
                        print('no conversion for', n)
                        node_list.append(n)
                    # convert constant node here, from fp32 to fp16
                    elif n.op_type == 'Constant' and n.attribute[0].t.data_type == onnx_proto.TensorProto.FLOAT:
                        n.attribute[0].t.data_type = onnx_proto.TensorProto.FLOAT16
                        # convert n.raw_data to float
                        data_fp32 = np.fromstring(n.attribute[0].t.raw_data, dtype='float32')
                        # convert float to float16
                        data_fp16 = np.float16(data_fp32)
                        # convert float16 to bytes and write back to raw_data
                        n.attribute[0].t.raw_data = data_fp16.tostring()
                    else:
            ...
            if isinstance(q, onnx_proto.GraphProto):
                for n in q.initializer:  # TensorProto type
                    if n.name not in block_initializer:
